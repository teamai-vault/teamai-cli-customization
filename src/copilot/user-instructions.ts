...
async function walkSource(directory: string, root: string, result: ManagedUserInstruction[]): Promise<void> {
  let entries;
  try {
...
async function walkTarget(directory: string, root: string, result: Map<string, Buffer>): Promise<void> {
  let entries;
  try {
    entries = await readdir(directory, { withFileTypes: true });
  } catch (error) {
